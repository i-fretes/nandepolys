...
import { serverNow, useMe, useOverlayBusy, useSettled, useStore } from '../store';
...
export default function ArenaDialog() {
  ...
  const open = state.turnPhase === 'ARENA' && !!a && !busy;
  return (
    <AnimatePresence>
      {open && a && (
...
// ---------------------------------------------------------------------------------------
/**
 * Esquivá el rayo. Tenés 4,5 s para elegir dónde pararte (podés cambiar de casilla las veces que
 * quieras hasta que se acabe el tiempo) y después los rayos quedan 2,4 s a la vista: la ronda
 * siguiente recién arranca cuando se apagan, así nunca se pierde tiempo de elección.
 */
function Rayo({ a, meId, participating }: GP) {
  ...
  const picked = (d.picked ?? []) as string[];
  const hits = (d.hits ?? null) as number[] | null;
  const lastPicks = (d.lastPicks ?? {}) as Record<string, number>;
  const revealing = d.phase === 'reveal' && !!hits;
  const meAlive = participating && a.alive.includes(meId!);
  const mine = meId ? picks[meId] : undefined;
  const until = (revealing ? d.revealUntil : d.pickUntil) as number | undefined;
  const left = Math.max(0, ((until ?? now) - now) / 1000);
  const total = revealing ? 2.4 : 4.5;
  const pct = Math.max(0, Math.min(100, (left / total) * 100));
  const urgent = !revealing && left <= 1.5;
  // Un tic por segundo mientras corre el reloj, para que se sienta la urgencia
  const lastBeep = useRef(-1);
  useEffect(() => {
    const sec = Math.ceil(left);
    if (!revealing && meAlive && sec !== lastBeep.current && sec > 0 && sec <= 3) { lastBeep.current = sec; sfx.countdown(); }
    if (revealing) lastBeep.current = -1;
  }, [left, revealing, meAlive]);
  // También se puede elegir con el teclado (1-9)
  useEffect(() => {
    if (!meAlive || revealing) return;
    const onKey = (e: KeyboardEvent) => {
      const n = Number(e.key);
      if (Number.isInteger(n) && n >= 1 && n <= size * size) { move(act, { cell: n - 1 }); sfx.tick(); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [meAlive, revealing, size, act]);

  const subtitle = revealing
    ? '⚡ ¡Cayeron los rayos! Mirá dónde pegaron…'
    : meAlive
      ? <>Ronda {a.round} · <b>Elegí dónde pararte</b> (podés cambiar hasta que se acabe el tiempo).</>
      : `Ronda ${a.round} · Estás afuera: mirá cómo se define.`;

  return (
    <div>
      <Header a={a} subtitle={subtitle} />
      <Players a={a} render={id => (a.alive.includes(id) ? (revealing ? '⚡' : picked.includes(id) ? '✔' : '…') : '⚡💀')} />

      {/* Reloj de la ronda: barra que se vacía + segundos */}
      <div className="mt-3 flex items-center gap-2">
        <div className={`rayo-bar ${revealing ? 'reveal' : urgent ? 'urgent' : ''}`}><span style={{ width: `${pct}%` }} /></div>
        <div className={`w-12 text-right font-mono text-lg font-black ${urgent ? 'text-red-600' : 'text-ink/60'}`} data-rayo-left>{left.toFixed(1)}s</div>
      </div>

      <div className="mx-auto mt-3 grid gap-2.5" style={{ gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))`, maxWidth: size * 128 }}>
        {Array.from({ length: size * size }, (_, i) => {
          const struck = revealing && hits?.includes(i);
          const standing = revealing
            ? Object.entries(lastPicks).filter(([, c]) => c === i).map(([id]) => state.players.find(p => p.id === id)!).filter(Boolean)
            : (mine === i && meId ? [state.players.find(p => p.id === meId)!] : []);
          return (
            <button key={i} data-cell={i} data-picked={mine === i ? '1' : undefined} disabled={!meAlive || revealing}
              onClick={() => { move(act, { cell: i }); sfx.tick(); }}
              className={`rayo-cell ${mine === i ? 'picked' : ''} ${struck ? 'struck' : ''} ${revealing && !struck ? 'safe' : ''}`}>
              {struck && <span className="bolt">⚡</span>}
              <span className="rayo-num">{i + 1}</span>
              <span className="flex flex-wrap justify-center gap-0.5">{standing.map(p => <span key={p.id} className="grid h-7 w-7 place-items-center rounded-full bg-white text-base" style={{ boxShadow: `0 0 0 2px ${p.color}` }}>{tokenEmoji(p.token)}</span>)}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-2 text-center text-sm font-semibold">
        {revealing ? <span className="text-ink/60">La próxima ronda arranca en {left.toFixed(1)} s</span>
          : !meAlive ? <span className="text-ink/50">Quedaste afuera.</span>
            : mine === undefined ? <span className="text-red-600">Todavía no elegiste: tocá una casilla (o apretá 1-{size * size}).</span>
              : <span className="text-emerald-700">Estás en la casilla {mine + 1}. Podés cambiar mientras quede tiempo.</span>}
      </div>
    </div>
  );
}
...
function Podium({ a }: { a: ArenaState }) {
  ...
  return (
    <div className="text-center">
      ...
      {a.game === 'trivia' && <TriviaResumen a={a} />}
...
// ---------------------------------------------------------------------------------------
/**
 * Resumen de la trivia de la Arena: las tres preguntas, la respuesta correcta y qué puso cada uno
 * (antes sólo se veía el podio y quedaba la duda de en qué se falló).
 */
function TriviaResumen({ a }: { a: ArenaState }) {
  const state = useStore(s => s.state)!;
  const history = ((a.data as D).history ?? []) as { q: string; options: string[]; correct: number; answers: Record<string, number> }[];
  const [abierto, setAbierto] = useState(0);
  if (!history.length) return null;
  const aciertos = (id: string) => history.filter(h => h.answers[id] === h.correct).length;
  return (
    <div className="mt-4 text-left" data-trivia-resumen>
      <div className="text-center text-sm font-black">🧠 Cómo le fue a cada uno</div>
      <div className="mt-2 flex flex-wrap justify-center gap-1.5">
        {a.players.map(id => {
          const p = state.players.find(x => x.id === id)!;
          const ok = aciertos(id);
          return (
            <span key={id} className={`chip ${ok === history.length ? 'bg-emerald-100 text-emerald-800' : ok === 0 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-ink/70'}`}>
              <span className="mr-1 grid h-4 w-4 place-items-center rounded-full bg-white text-[10px]" style={{ boxShadow: `0 0 0 2px ${p.color}` }}>{tokenEmoji(p.token)}</span>
              {p.name}: {ok}/{history.length} · {a.scores[id] ?? 0} pts
            </span>
          );
        })}
      </div>
      <div className="mt-3 space-y-2">
        {history.map((h, qi) => (
          <div key={qi} className="rounded-xl border border-black/10 bg-white">
            <button className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm font-bold" onClick={() => setAbierto(abierto === qi ? -1 : qi)}>
              <span className="chip bg-slate-100 text-[10px]">{qi + 1}</span>
              <span className="flex-1">{h.q}</span>
              <span className="text-xs text-ink/40">{abierto === qi ? '▾' : '▸'}</span>
            </button>
            {abierto === qi && (
              <div className="space-y-1 px-3 pb-2">
                {h.options.map((o, i) => {
                  const who = a.players.filter(id => h.answers[id] === i);
                  const right = h.correct === i;
                  if (!right && who.length === 0) return null;
                  return (
                    <div key={i} className={`flex items-center gap-2 rounded-lg px-2 py-1 text-xs ${right ? 'bg-emerald-50' : 'bg-red-50'}`}>
                      <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[10px] font-black text-white ${right ? 'bg-emerald-500' : 'bg-red-400'}`}>{right ? '✔' : '✘'}</span>
                      <span className={`flex-1 ${right ? 'font-bold' : ''}`}>{o}</span>
                      <span className="flex flex-wrap justify-end gap-1">
                        {who.map(id => { const p = state.players.find(x => x.id === id)!; return <span key={id} className="rounded-full bg-white px-1.5 py-0.5">{tokenEmoji(p.token)} {p.name}</span>; })}
                      </span>
                    </div>
                  );
                })}
                {a.players.some(id => h.answers[id] === undefined) && (
                  <div className="px-2 text-[11px] text-ink/50">No respondieron: {a.players.filter(id => h.answers[id] === undefined).map(id => state.players.find(x => x.id === id)?.name).join(', ')}</div>
                )}
              </div>
            )}
          </div>
