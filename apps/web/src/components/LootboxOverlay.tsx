...
import { useFxOverlayReady, useSettled, useStore } from '../store';
...
export default function LootboxOverlay() {
  // Espera su turno: nunca se muestra encima de una carta, de la ficha en movimiento ni de una ventana de mesa
  const ready = useSettled(useFxOverlayReady());
  const box = useStore(s => s.lootbox);
  return <AnimatePresence>{ready && box && <LootboxInner key={box.id} />}</AnimatePresence>;
}

function LootboxInner() {
  const box = useStore(s => s.lootbox);
  ...
  // Caja cerrada: si el dueño no la abre en 9 s, se abre sola
  useEffect(() => {
    if (!box) return;
    setPhase('closed');
    if (box.openedAt) return;
    const t = setTimeout(() => useStore.setState(s => (s.lootbox && !s.lootbox.openedAt ? { lootbox: { ...s.lootbox, openedAt: Date.now() } } : {})), 9000);
  ...
  useEffect(() => {
    ...
    const start = setTimeout(() => {
      ...
      timers.push(setTimeout(() => setLootbox(null), SPIN_MS + 3800));
    }, delay);
