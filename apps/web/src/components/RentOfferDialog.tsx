...
import { DadosFinal, TriviaFinal, type Detail } from './ChallengeDialog';
...
export default function RentOfferDialog() {
  ...
  const open = ((state.turnPhase === 'RENT_OFFER' && !!o && !busy) || !!show);
  if (!open) return <Modal open={false} />;
  const secs = deadline ? Math.max(0, Math.ceil((deadline - now) / 1000)) : null;

  if (show && lastDon) {
    const d = lastDon.data as { dice?: [number, number]; win: boolean; amount: number; tie?: boolean; kind?: ChallengeKind; detail?: Detail };
    ...
    const emoji = (id: string | null | undefined) => { const p = state.players.find(x => x.id === id); return p ? tokenEmoji(p.token) : ''; };
    const veredicto = (
      <div className={`reveal mt-3 text-center text-xl font-black ${d.win ? 'text-emerald-600' : d.tie ? 'text-ink' : 'text-red-600'}`}>
        {d.win ? '¡Ganó el doble o nada! No paga nada.' : d.tie ? 'Empate: paga el alquiler normal.' : `Perdió: paga el doble, ${money(d.amount)}`}
      </div>
    );
    // Si se definió con dados o trivia, primero se ve cómo salió y después el veredicto
    if (d.detail?.kind === 'dados') return <Modal open width="max-w-md"><h2 className="mb-2 text-center text-xl font-black">🎲 Doble o nada</h2><DadosFinal det={d.detail} name={name} emoji={emoji}>{veredicto}</DadosFinal></Modal>;
    if (d.detail?.kind === 'trivia') return <Modal open width="max-w-lg"><h2 className="mb-2 text-center text-xl font-black">🧠 Doble o nada</h2><TriviaFinal det={d.detail} name={name} emoji={emoji}>{veredicto}</TriviaFinal></Modal>;
    return (
