  ...
  const mgmt = (
    <>
      ...
      {(() => {
        ...
        const inPanel = !!pt && (pt.toId === me.id || pt.fromId === me.id || state.tradeRivals?.some(r => r.fromId === me.id));
        const canImprove = !!pt && pt.fromId === me.id && (state.tradeRivals?.length ?? 0) > 0 && !state.tradeImproved;
        return (
          <button className={`btn-ghost btn-sm ${legal.has('TRADE_BUTT_IN') ? 'breathe !border-amber-400' : ''}`} onClick={() => setDialog('trade')}
            disabled={inPanel ? !canImprove : !(legal.has('TRADE_PROPOSE') || legal.has('TRADE_BUTT_IN'))}
            title={inPanel ? (canImprove ? 'Se metió un metiche: podés mejorar tu oferta' : 'La propuesta está en el panel Intercambios (columna izquierda)') : legal.has('TRADE_BUTT_IN') ? 'Hay un trato en la mesa: podés meterte de metiche' : 'Proponer un intercambio'}>
            {legal.has('TRADE_BUTT_IN') ? '🕵️ Meterme' : canImprove ? '🤝 Mejorar oferta' : '🤝 Intercambiar'}
          </button>
        );
      })()}
