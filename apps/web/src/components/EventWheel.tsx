...
export default function EventWheel() {
  // Espera su turno: nunca encima de una carta, de la ficha en movimiento ni de una ventana de mesa
  const ready = useSettled(useFxOverlayReady('event')); // la ruleta espera también a la caja sorpresa
  const spin = useStore(s => s.eventSpin);
  return <AnimatePresence>{ready && spin && <EventWheelInner key={spin.id} />}</AnimatePresence>;
}

function EventWheelInner() {
  const spin = useStore(s => s.eventSpin);
