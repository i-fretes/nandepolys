...
export default function CasinoDialog() {
  ...
  const overlayBusy = !useSettled(!useOverlayBusy());
  const open = state.turnPhase === 'CASINO' && !!c && !overlayBusy;
  ...
  // Cuando llega MI resultado, la vista pasa a ese juego. El resultado de otro jugador no me cambia
  // la pantalla: antes pisaba el mío y parecía que el número cambiaba solo.
  const mineResult = last && last.data.playerId === me ? last : null;
  useEffect(() => {
    if (mineResult?.data.game) setGame(mineResult.data.game as Game);
    setBusy(false);
  }, [mineResult?.id]); // eslint-disable-line react-hooks/exhaustive-deps

  if (!open || !c || !player) return <Modal open={false} />;
  const showResult = mineResult && (mineResult.data.game === game) && iPlayed;
    ...
    <Modal open width="max-w-2xl">
      <div className="-m-5 rounded-2xl bg-gradient-to-br from-[#2a0845] via-[#4a0d67] to-[#7a1b4d] p-5 text-white">
        ...
        <div className="mt-3 rounded-2xl bg-black/30 p-3">
          {game === 'ruleta' && <Ruleta result={showResult ? mineResult!.data : null} />}
          {game === 'quiniela' && <Quiniela pick={pick} setPick={setPick} disabled={!mine || iPlayed} result={showResult ? mineResult!.data : null} />}
          {game === 'carrera' && <Carrera cart={cart} setCart={setCart} disabled={!mine || iPlayed} result={showResult ? mineResult!.data : null} />}
...
function Ruleta({ result }: { result: Record<string, unknown> | null }) {
  ...
  // Sin desvío al azar: la celda que salió queda exactamente bajo el marcador. Con el desvío, el
  // marcador quedaba entre dos celdas y se veía una verde aunque el resultado fuera rojo.
  useReelSpin(stripRef, roll !== null, { cell: CELL, winCell: WIN, ms: SPIN_MS, direction: 'left' });
    ...
    <div>
      <div className="reel-window mx-auto" style={{ maxWidth: 560 }}>
        ...
        <div ref={stripRef} className="reel-strip">
          {cells.map((n, i) => (
            <div key={i} className={`reel-cell ${n <= RULETA_WIN_CHANCE ? 'win' : 'lose'} ${done && i === WIN ? 'hit' : ''} ${done && i !== WIN ? 'faded' : ''}`} data-cell={i}>
