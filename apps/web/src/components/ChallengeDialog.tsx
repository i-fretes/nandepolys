...
export default function ChallengeDialog() {
  ...
  useEffect(() => {
    if (!lastDone) return;
    setResult(lastDone);
    // Cada final se cuenta con su tiempo: primero *qué pasó* y recién después el ganador
    const bj = (lastDone.data as { bj?: { hands: Record<string, number[]>; house: string } }).bj;
    const kind = (lastDone.data as { kind?: string }).kind;
    const ms = bj ? 2600 + 1000 * Math.max(0, bj.hands[bj.house].length - 1) + 9000
      : kind === 'dados' ? 9000 : kind === 'trivia' ? 11000 : kind === 'ppt' ? 8000 : 6000;
    const id = setTimeout(() => setResult(r => (r?.id === lastDone.id ? null : r)), ms);
  ...
  if (result && !c) {
    const d = result.data as { winner: string | null; loser?: string; amount: number; kind: ChallengeKind; bj?: NonNullable<Ch>['data']['bj']; detail?: Detail };
    const iWon = d.winner === me, iLost = d.loser === me;
    const splash = (compact: boolean) => (
      <div className={`vs-splash text-center ${compact ? 'mt-3' : ''}`} data-challenge-splash>
        <div className={compact ? 'text-5xl' : 'text-6xl'}>{d.winner ? '🏆' : '🤝'}</div>
        <h2 className={`mt-1 text-2xl font-black ${iWon ? 'text-emerald-600' : iLost ? 'text-red-600' : ''}`}>
          {d.winner ? `¡Ganó ${name(d.winner)}!` : 'Sin ganador'}
        </h2>
        <p className="mt-1 text-sm text-ink/70">{result.text}</p>
        {d.amount > 0 && <div className="mt-2 text-3xl font-black text-emerald-700">+{money(d.amount)}</div>}
        <button className="btn-ghost mt-3 w-full" onClick={() => setResult(null)}>Cerrar</button>
      </div>
    );
    const det = d.detail;
    return (
      <Modal open onClose={() => setResult(null)} width={d.bj || det?.kind === 'trivia' ? 'max-w-lg' : 'max-w-md'}>
        {d.bj ? <BjFinal bj={d.bj} name={name}>{splash(true)}</BjFinal>
          : det?.kind === 'dados' ? <DadosFinal det={det} name={name} emoji={emoji}>{splash(true)}</DadosFinal>
          : det?.kind === 'trivia' ? <TriviaFinal det={det} name={name} emoji={emoji}>{splash(true)}</TriviaFinal>
          : det?.kind === 'ppt' ? <PptFinal det={det} name={name}>{splash(true)}</PptFinal>
          : splash(false)}
      </Modal>
    );
  }
...
/**
 * Mesa final del blackjack, contada con calma: primero se ve la mesa como quedó (la banca con su carta
 * tapada), después la banca da vuelta la carta, pide de a una carta por segundo, aparece el resultado
 * y recién al final el cartel del ganador (children). Así se entiende por qué ganó quien ganó.
 */
function BjFinal({ bj, name, children }: { bj: NonNullable<NonNullable<Ch>['data']['bj']>; name: (id: string | null | undefined) => string; children?: React.ReactNode }) {
  const hs = bj.hands[bj.house], my = bj.hands[bj.bettor];
  const myV = bjValue(my);
  const bettorBusted = myV.total > 21;
  // step 0: carta tapada · 1: banca destapada (2 cartas) · 2..: cartas extra de la banca · N+1: resultado · N+2: ganador
  const extra = Math.max(0, hs.length - 2);
  const resultStep = 2 + extra, doneStep = resultStep + 1;
  const [step, setStep] = useState(0);
  useEffect(() => {
    const timers: ReturnType<typeof setTimeout>[] = [];
    let t = 1100;
    timers.push(setTimeout(() => { setStep(1); sfx.card(); }, t));
    for (let i = 0; i < extra; i++) { t += 1000; const st = 2 + i; timers.push(setTimeout(() => { setStep(st); sfx.card(); }, t)); }
    t += 900; timers.push(setTimeout(() => { setStep(resultStep); if (bj.result === 'house') sfx.lose(); else if (bj.result === 'push') sfx.notify(); else sfx.coin(); }, t));
    t += 1200; timers.push(setTimeout(() => setStep(doneStep), t));
    return () => timers.forEach(clearTimeout);
  }, [bj.house, bj.bettor]); // eslint-disable-line react-hooks/exhaustive-deps
  const shown = step === 0 ? hs.slice(0, 2) : hs.slice(0, Math.min(hs.length, 2 + Math.max(0, step - 1)));
  const hidden = step === 0;
  const hv = hidden ? bjValue([hs[0]]) : bjValue(shown);
  const houseText = hidden ? (bettorBusted ? 'se pasó el que apostaba: la banca destapa' : 'da vuelta su carta…')
    : step < resultStep && shown.length < hs.length ? `tiene ${hv.total}: pide…`
    : step < resultStep ? `tiene ${hv.total}${hv.total > 21 ? ' · se pasó' : ' · se planta'}`
    : `${hv.total}${hv.total > 21 ? ' · se pasó' : ''}`;
  return (
    <div>
      <div className="bjtable mb-1" data-bj-final data-bj-step={step}>
        <div className="bj-row">
          <div className="bj-label"><b>{name(bj.house)}</b> · la banca · {houseText}</div>
          <div className="bj-hand">{shown.map((card, i) => <BjCard key={`${card}-${i}`} card={card} hidden={hidden && i === 1} delay={step === 0 ? i * 90 : 0} />)}</div>
        </div>
        <div className="bj-mid">
          <span className="bj-stake">Se jugó {money(bj.stake)}{bj.doubled ? ' (doblada)' : ''}</span>
          {step >= resultStep && bj.result && (
            <span className={`bj-result reveal ${bj.result === 'house' ? 'lose' : bj.result === 'push' ? 'push' : 'win'}`}>
              {bj.result === 'blackjack' ? '¡BLACKJACK! paga 3 a 2' : bj.result === 'bettor' ? `Gana ${name(bj.bettor)}` : bj.result === 'push' ? 'Empate: nadie paga' : 'Gana la banca'}
            </span>
          )}
        ...
        <div className="bj-row">
          <div className="bj-label"><b>{name(bj.bettor)}</b> · apuesta · <b>{myV.total}</b>{bettorBusted ? ' · se pasó' : ''}</div>
          <div className="bj-hand">{my.map((card, i) => <BjCard key={`${card}-${i}`} card={card} delay={step === 0 ? i * 90 : 0} />)}</div>
        </div>
      </div>
      {step >= doneStep ? children : <div className="mt-3 h-10 text-center text-sm text-ink/50">{step < resultStep ? 'La banca juega…' : ''}</div>}
    </div>
  );
}

// --- Finales contados con calma ----------------------------------------------------------------
/** Foto del final que manda el motor (qué tiró cada uno, qué respondió cada uno…). */
export type Detail = {
  kind: ChallengeKind; fromId: string; toId: string;
  rolls?: Record<string, [number, number]>;
  ties?: Record<string, [number, number]>;
  round?: number;
...
/** Avanza de paso cada `ms` hasta `steps`; sirve para contar el final de a poco. */
function useSteps(steps: number, ms: number[]) {
  const [step, setStep] = useState(0);
  useEffect(() => {
    const timers: ReturnType<typeof setTimeout>[] = [];
    let t = 0;
    for (let i = 1; i <= steps; i++) { t += ms[i - 1] ?? 900; timers.push(setTimeout(() => setStep(i), t)); }
    return () => timers.forEach(clearTimeout);
...
/**
 * Duelo de dados: se ven las dos tiradas (y los empates anteriores, si los hubo), después se marca
 * la más alta y recién al final aparece el ganador.
 */
export function DadosFinal({ det, name, emoji, children }: { det: Detail; name: (id: string | null | undefined) => string; emoji: (id: string | null | undefined) => string; children?: React.ReactNode }) {
  const ids = [det.fromId, det.toId];
  const rolls = det.rolls ?? {};
  const sum = (id: string) => (rolls[id] ? rolls[id][0] + rolls[id][1] : 0);
  const hi = sum(ids[0]) === sum(ids[1]) ? null : sum(ids[0]) > sum(ids[1]) ? ids[0] : ids[1];
  const step = useSteps(2, [1500, 1500]);   // 0: tiradas · 1: se marca la más alta · 2: ganador
  useEffect(() => { if (step === 1) sfx.dice(); }, [step]);
  return (
    <div data-dados-final data-step={step}>
      <div className="text-center text-sm font-semibold text-ink/50">🎲 Duelo de dados{det.round && det.round > 1 ? ` · se definió en la ronda ${det.round}` : ''}</div>
      <div className="mt-3 grid grid-cols-2 gap-3">
        {ids.map(id => {
          const r = rolls[id];
          const won = step >= 1 && hi === id;
          const lost = step >= 1 && hi !== null && hi !== id;
          return (
            <div key={id} className={`rounded-2xl border-2 p-3 text-center transition ${won ? 'border-emerald-500 bg-emerald-50' : lost ? 'border-black/10 bg-slate-50 opacity-60' : 'border-black/10 bg-white'}`}>
              <div className="text-sm font-bold">{emoji(id)} {name(id)}</div>
              <div className="my-2 flex justify-center gap-2">
                <Die3D value={r?.[0] ?? null} rolling={false} size="52px" />
                <Die3D value={r?.[1] ?? null} rolling={false} size="52px" />
              </div>
              <div className={`text-3xl font-black ${won ? 'text-emerald-700' : ''}`}>{sum(id) || '?'}</div>
              {won && <div className="mt-1 text-xs font-black uppercase tracking-wide text-emerald-700">la más alta</div>}
            </div>
          );
        })}
      </div>
      {det.ties && Object.keys(det.ties).length > 0 && (
        <div className="mt-2 rounded-lg bg-amber-50 px-3 py-1 text-center text-xs font-bold text-amber-800">
          Antes habían empatado {(det.ties[ids[0]]?.[0] ?? 0) + (det.ties[ids[0]]?.[1] ?? 0)} a {(det.ties[ids[1]]?.[0] ?? 0) + (det.ties[ids[1]]?.[1] ?? 0)}
        </div>
      )}
      {step >= 2 ? children : <div className="mt-3 h-10 text-center text-sm text-ink/50">{step === 0 ? 'Las dos tiradas…' : 'Comparando…'}</div>}
    </div>
  );
}

/**
 * Trivia: queda la pregunta a la vista, se marca la respuesta correcta en verde y debajo de cada
 * opción se ve quién puso qué. Recién después aparece el ganador.
 */
export function TriviaFinal({ det, name, emoji, children }: { det: Detail; name: (id: string | null | undefined) => string; emoji: (id: string | null | undefined) => string; children?: React.ReactNode }) {
  const q = det.question;
  const answers = det.answers ?? {};
  const step = useSteps(2, [1600, 1800]);   // 0: pregunta y respuestas · 1: se marca la correcta · 2: ganador
  useEffect(() => { if (step === 1) sfx.notify(); }, [step]);
  if (!q) return <>{children}</>;
  return (
    <div data-trivia-final data-step={step}>
      <div className="text-center text-sm font-semibold text-ink/50">🧠 Trivia paraguaya{det.qIndex !== undefined ? ` · pregunta ${(det.qIndex as number) + 1}` : ''}</div>
      <div className="mt-2 rounded-2xl bg-slate-800 p-3 text-center text-base font-bold text-white">{q.q}</div>
      <div className="mt-2 grid gap-1.5">
        {q.options.map((o, i) => {
          const who = Object.entries(answers).filter(([, v]) => v === i).map(([id]) => id);
          const right = step >= 1 && det.correct === i;
          const wrong = step >= 1 && det.correct !== i && who.length > 0;
          return (
            <div key={i} className={`flex items-center gap-2 rounded-xl border-2 px-3 py-2 text-sm transition ${right ? 'border-emerald-500 bg-emerald-50' : wrong ? 'border-red-300 bg-red-50' : 'border-black/10 bg-white'}`}>
              <span className={`grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-black ${right ? 'bg-emerald-500 text-white' : wrong ? 'bg-red-400 text-white' : 'bg-slate-100'}`}>{right ? '✔' : wrong ? '✘' : 'ABCD'[i]}</span>
              <span className={`flex-1 ${right ? 'font-bold' : ''}`}>{o}</span>
              <span className="flex flex-wrap justify-end gap-1">
                {who.map(id => <span key={id} className={`chip text-[11px] ${right ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-700'}`}>{emoji(id)} {name(id)}</span>)}
              </span>
            </div>
          );
        })}
      </div>
      {step >= 1 && Object.keys(answers).length < 2 && (
        <div className="mt-2 text-center text-xs text-ink/50">
          {[det.fromId, det.toId].filter(id => answers[id] === undefined).map(id => name(id)).join(' y ')} no llegó a responder.
        </div>
      )}
      {step >= 2 ? children : <div className="mt-3 h-10 text-center text-sm text-ink/50">{step === 0 ? 'Esto respondió cada uno…' : 'La respuesta correcta era…'}</div>}
    </div>
  );
}

/** Piedra, papel o tijera: las rondas jugadas y el marcador antes del ganador. */
function PptFinal({ det, name, children }: { det: Detail; name: (id: string | null | undefined) => string; children?: React.ReactNode }) {
  const step = useSteps(1, [1800]);
  const rounds = det.rounds ?? [];
  return (
    <div data-ppt-final data-step={step}>
      <div className="text-center text-sm font-semibold text-ink/50">✊ Piedra, papel o tijera · mejor de 3</div>
      <div className="mt-2 flex items-center justify-center gap-4 text-2xl font-black">
        <span>{name(det.fromId)} {det.score?.[det.fromId] ?? 0}</span>
        <span className="text-sm text-ink/40">vs</span>
        <span>{det.score?.[det.toId] ?? 0} {name(det.toId)}</span>
      </div>
      <div className="mt-2 space-y-1">
        {rounds.map((r, i) => (
          <div key={i} className="flex items-center justify-center gap-3 rounded-lg bg-cream px-3 py-1 text-sm">
            <span className="text-xl">{PPT.find(p => p.id === r.a)?.icon}</span>
            <span className="text-xs font-semibold text-ink/60">{r.winner ? `punto ${name(r.winner)}` : 'empate'}</span>
            <span className="text-xl">{PPT.find(p => p.id === r.b)?.icon}</span>
          </div>
        ))}
      </div>
      {step >= 1 ? children : <div className="mt-3 h-10 text-center text-sm text-ink/50">Así se jugó…</div>}
    </div>
  );
}
