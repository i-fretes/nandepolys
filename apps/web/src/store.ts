    ...
    if (events.length) {
      ...
      for (const e of events) {
        ...
        if (e.type === 'casino_result' && e.playerId) {
          ...
          patch.lastCasino = { id: ++fxCounter, data: { ...(e.data ?? {}), playerId: e.playerId } };
        }
        if (e.type === 'casino_double' && e.playerId) patch.lastCasino = { id: ++fxCounter, data: { game: 'doble', ...e.data, playerId: e.playerId } };
...
/**
 * Ventanas "de mesa" que manda el servidor (casino, alquiler doble o nada, desafío, arena, subasta, duelo).
 * Tienen sus propios relojes, así que van antes que la caja sorpresa y la ruleta de eventos.
 */
const TABLE_PHASES = new Set(['CASINO', 'RENT_OFFER', 'CHALLENGE', 'ARENA', 'AUCTION', 'DUEL']);
export const useTableBusy = () => useStore(s => !!s.state && (TABLE_PHASES.has(s.state.turnPhase) || !!s.state.challenge || s.state.phase === 'FINISHED'));
/**
 * ¿Hay algo en pantalla que las ventanas de mesa deben esperar? La ficha moviéndose o una carta leyéndose.
 * Con esto nunca aparece una ventana encima de otra: primero termina de moverse la ficha, después se lee
 * la carta, después la ventana de mesa y al final la caja sorpresa / ruleta de eventos.
 */
export const useOverlayBusy = () => useStore(s => s.moving !== null || !!s.cardModal);
/** Devuelve true sólo cuando `v` lleva `ms` milisegundos seguidos en true (deja terminar la animación de cierre de la ventana anterior). */
export function useSettled(v: boolean, ms = 450) {
  const [ok, setOk] = useState(false);
  useEffect(() => {
    if (!v) { setOk(false); return; }
    const t = setTimeout(() => setOk(true), ms);
    return () => clearTimeout(t);
  }, [v, ms]);
  return ok && v;
}
/** La caja sorpresa y la ruleta esperan a todo lo demás (no tienen apuro: el premio ya está decidido). */
export const useFxOverlayReady = (after: 'lootbox' | 'event' = 'lootbox') => useStore(s => !!s.state && s.moving === null && !s.cardModal && !(TABLE_PHASES.has(s.state.turnPhase) || !!s.state.challenge || s.state.phase === 'FINISHED') && (after === 'lootbox' || !s.lootbox));
