...
function arenaBot(s: GameState): Action | null {
  ...
  switch (a.game) {
    ...
    case 'rayo': {
      // Los bots se toman un momento para elegir (antes elegían al instante y la ronda se resolvía sola)
      if (d.phase !== 'pick') return null;
      const since = now - (d.roundStartedAt ?? now);
      if (since < 900) return null;
      const b = bots.find(id => a.alive.includes(id) && d.picks?.[id] === undefined);
      if (!b) return null;
      const n = (d.gridSize ?? 3) ** 2;
      return { type: 'ARENA_MOVE', playerId: b, now, payload: { cell: Math.floor(hash(s, b.length + (a.round ?? 0) * 7) * n) } };
    }
