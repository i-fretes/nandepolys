...
- v1.6.3: **mini-juegos y desafíos que se entienden**. *Esquivá el rayo*: la ronda dura 4,5 s de verdad (antes el reloj arrancaba mientras todavía se veían los rayos de la ronda anterior y quedaba poco más de un segundo), hay **barra de tiempo y cuenta regresiva**, las casillas están numeradas, se puede **cambiar de casilla** hasta que se acaba el tiempo (o elegir con las teclas 1-9), los rayos quedan 2,4 s a la vista y recién ahí empieza la ronda siguiente; además nadie ve la casilla del otro mientras se elige (sólo que ya eligió) y los bots se toman un momento. *Duelo de dados*: al terminar se ven las dos tiradas, se marca en verde la más alta y recién después aparece el ganador. *Trivia* (desafío): queda la pregunta en pantalla, se marca la correcta y **se ve qué puso cada uno**, y después el ganador; en la **Arena** el podio suma un resumen con las tres preguntas, la respuesta correcta, qué contestó cada jugador y cuántas acertó. *Casino*: la ruleta frena **exactamente** sobre la casilla que salió (antes quedaba corrida y se veía verde al lado de un resultado rojo), las demás casillas se apagan al frenar, y el resultado de otro jugador ya no pisa el tuyo en pantalla.
- v1.6.2: el **final del blackjack** se cuenta con calma: al plantarse, la mesa queda como estaba (la banca con su carta tapada), la banca da vuelta la carta, pide de a una carta por segundo mostrando cuánto suma, aparece el resultado y recién después el cartel del ganador, que queda 9 s (o hasta que lo cerrás). Antes saltaba directo al ganador y no se entendía por qué.
- v1.6.1: **panel Intercambios** en la columna izquierda (entre En vivo y las misiones): ahí llegan las propuestas (con Aceptar / Rechazar), ahí ves la tuya "esperando" con los metiches que se metieron y el botón para mejorarla, y ahí aparece el trato ajeno con el botón para meterte; ya no se abre ninguna ventana encima del tablero para responder — la ventana queda sólo para armar una oferta. **Dados planos y minimalistas** (cuadrado blanco con puntos, cambian de cara y se sacuden apenas mientras ruedan) en el tablero, el casino, el doble o nada y los desafíos; se fue el dado 3D.
- v1.6.0: **la partida se juega en la casa de alguien** — el tablero está apoyado sobre una mesa de tablones de madera con una alfombra tipo ñandutí debajo, el piso de parquet alrededor y una lámpara cálida encima; sobre la mesa aparecen (sólo si hay lugar, nunca encima del tablero) los dados, el mazo de Suerte, un plato de chipa, el termo con la guampa y billetes. Los paneles quedan igual que antes. El tablero se ve **de lejos** (~78 % del alto) y las casillas se leen con la **lupa**: al pasar el mouse la casilla se agranda hacia adentro del tablero; con 🔍 se lo trae "de cerca" como antes. **Vista de mesa arreglada**: ahora gira sobre el borde de abajo, así nunca se sale de su columna ni pisa los paneles o la barra de acciones (hay una prueba que lo mide en 4 tamaños de pantalla: `node e2e/casa.mjs`). **Pantalla completa** de verdad (usa la del navegador) con una barra arriba y un botón rojo grande "Salir de pantalla completa", otro botón debajo de la barra de acciones y la tecla Esc. **Nada encima de nada**: las ventanas se turnan — primero termina de moverse la ficha, después se lee la carta, después la ventana de mesa (desafío, casino, alquiler doble o nada, arena, subasta, duelo) y al final la caja sorpresa y la ruleta de eventos, que esperan su momento sin perderse (`node e2e/ventanas.mjs` reproduce el caso de la caja encima del ¡Desafío obligatorio!). **Música nueva**: jazz relajado generado en vivo (piano eléctrico con acordes de séptima, contrabajo, escobillas, melodía suave y reverberación); si preferís otra, poné un archivo `apps/web/public/musica.mp3` y el juego lo usa en loop en su lugar. Se **quitó el dado ñandú** (no aportaba y confundía).
...
- **Estilo visual:** `apps/web/src/index.css` y `tailwind.config.js`. El fondo de la casa (mesa, alfombra, piso, objetos) está en `apps/web/src/components/HouseScene.tsx`.
- **Música de fondo:** por defecto es jazz generado en vivo (`apps/web/src/sound.ts`). Para usar una canción tuya, copiá un mp3 (libre de derechos) como `apps/web/public/musica.mp3` y volvé a desplegar: el juego lo detecta solo y lo reproduce en loop, bajito.
