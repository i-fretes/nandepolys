...
import {
  ...
  type ChallengeState, type GlobalEventId, type Group, type Player, type PlayerStats, type PptChoice, type PropertyTile, type TokenId, type TradeSide, type TradeState, type TurnPhase,
} from './types';
...
/** Colores con menos aterrizajes hasta ahora. */
...
// "Esquivá el rayo": tiempo para elegir casilla y cuánto quedan a la vista los rayos antes de la
// ronda siguiente. Antes la ronda duraba 3 s y arrancaba mientras todavía se mostraban los rayos de
// la anterior: quedaba poco más de un segundo para elegir y no se llegaba.
export const RAYO_PICK_MS = 4500;
export const RAYO_REVEAL_MS = 2400;
/** Cuando ya eligieron todos, se acelera pero se deja un momento para cambiar de idea. */
export const RAYO_ALL_PICKED_MS = 1200;
export const RAYO_MAX_ROUNDS = 8;
...
/**
 * Foto del final del desafío: las dos tiradas, la pregunta con lo que puso cada uno, las manos…
 * Viaja en el evento para que la pantalla del resultado pueda mostrar *por qué* ganó quien ganó.
 */
function challengeDetail(c: ChallengeState): Record<string, unknown> {
  return {
    kind: c.kind, fromId: c.fromId, toId: c.toId,
    rolls: c.data.rolls ? { ...(c.data.rolls as Record<string, [number, number]>) } : undefined,
    ties: c.data.lastRolls ? { ...(c.data.lastRolls as Record<string, [number, number]>) } : undefined,
    round: c.data.round,
    question: c.data.question, answers: c.data.answered ? { ...(c.data.answered as Record<string, number>) } : undefined,
    correct: c.secret.answer, qIndex: c.data.qIndex,
    rounds: c.data.rounds, score: c.data.score,
  };
}
...
function finishChallenge(ctx: Ctx, winnerId: string | null, reason: string) {
  ...
  const detail = challengeDetail(c);
  ...
  if (c.rent) {
    ...
    if (winnerId === r.payerId) {
      payer.stats.donWins++;
      emit(ctx, 'rent_don_roll', `🎉 ${payer.name} ganó el doble o nada (${reason}) y no paga nada en ${name}.`, r.payerId, { win: true, amount: r.rent, to: r.ownerId, kind: c.kind, bj: c.data.bj, detail });
      return finishResolution(ctx);
    }
    if (!winnerId) {
      emit(ctx, 'rent_don_roll', `Empate en el doble o nada: ${payer.name} paga el alquiler normal.`, r.payerId, { win: false, amount: r.rent, to: r.ownerId, tie: true, detail });
      return payRent(ctx, payer, owner, r.rent, name);
    }
    emit(ctx, 'rent_don_roll', `💸 ${owner.name} ganó el doble o nada (${reason}): ${payer.name} paga ${fmt(r.rent * 2)}.`, r.payerId, { win: false, amount: r.rent * 2, to: r.ownerId, kind: c.kind, bj: c.data.bj, detail });
  ...
  if (winnerId) {
    ...
    emit(ctx, 'challenge_done', `¡${winner.name} ganó el desafío (${reason}) y cobra ${fmt(amt)} de ${loser.name}!`, winnerId, { kind: c.kind, winner: winnerId, loser: loserId, amount: amt, to: winnerId, bj: c.data.bj, detail });
  } else {
    emit(ctx, 'challenge_done', `El desafío terminó sin ganador (${reason}). Nadie paga.`, undefined, { kind: c.kind, winner: null, amount: 0, bj: c.data.bj, detail });
  }
  s.challenge = null;
  s.turnPhase = c.returnPhase;
...
function arenaSetup(ctx: Ctx) {
  ...
  switch (a.game) {
    case 'trivia': { sec.used = []; d.qIndex = 0; d.history = []; arenaNextTrivia(ctx); return; }
    ...
    case 'rayo': {
      d.gridSize = 3; d.picks = {}; d.hits = null; d.lastPicks = {}; d.picked = [];
      d.phase = 'pick'; d.roundStartedAt = a.startedAt; d.pickUntil = (a.startedAt ?? 0) + RAYO_PICK_MS;
      return;
    }
...
function arenaMove(ctx: Ctx, playerId: string, now: number, payload: Record<string, unknown>) {
  ...
  switch (a.game) {
    ...
    case 'rayo': {
      if (!a.alive.includes(playerId)) throw new RuleError('Estás eliminado.');
      if (d.phase !== 'pick') throw new RuleError('Esperá a la próxima ronda.');
      ...
      picks[playerId] = cell;                       // se puede cambiar de casilla hasta que se acabe el tiempo
      d.picked = a.alive.filter(id => picks[id] !== undefined);
      // Si ya eligieron todos, no se resuelve de golpe: queda un momento para arrepentirse
      if ((d.picked as string[]).length === a.alive.length) d.pickUntil = Math.min(d.pickUntil as number, now + RAYO_ALL_PICKED_MS);
      return;
    }
    case 'penales': {
...
function arenaTriviaReveal(ctx: Ctx, now: number) {
  ...
  // Guardamos la pregunta con lo que puso cada uno, para el resumen del final
  const qq = d.question as { q: string; options: string[] };
  (d.history as { q: string; options: string[]; correct: number; answers: Record<string, number> }[]).push({
    q: qq.q, options: qq.options, correct: a.secret.answer as number, answers: { ...(d.answered as Record<string, number>) },
  });
...
/** Caen los rayos: se destapan las casillas de todos y quedan a la vista unos segundos. */
function arenaRayoResolve(ctx: Ctx, now: number) {
  ...
  const n = size * size;
  const picks = d.picks as Record<string, number>;
  // Al que se quedó sin elegir (se le acabó el tiempo) lo para el azar donde sea
  ...
  d.phase = 'reveal';
  d.revealUntil = now + RAYO_REVEAL_MS;
  d.over = a.alive.length <= 1 || a.round >= RAYO_MAX_ROUNDS;   // se termina recién cuando se vieron los rayos
}

/** Nueva ronda: recién acá arranca el tiempo para elegir (nunca mientras se muestran los rayos). */
function arenaRayoNextRound(ctx: Ctx, now: number) {
  const a = ctx.s.arena!;
  const d = a.data;
  a.round++;
  if (a.round === 5 && (d.gridSize as number) > 2) d.gridSize = 2;  // menos lugares: se define más rápido
  d.picks = {}; d.picked = []; d.hits = null; d.phase = 'pick';
  d.roundStartedAt = now; d.pickUntil = now + RAYO_PICK_MS;
}
...
function arenaTick(ctx: Ctx, now: number) {
  ...
  switch (a.game) {
    ...
    case 'rayo': {
      if (d.phase === 'reveal') {
        if (now >= (d.revealUntil as number)) { if (d.over) return arenaFinish(ctx); arenaRayoNextRound(ctx, now); }
        return;
      }
      if (now >= (d.pickUntil as number)) arenaRayoResolve(ctx, now);
      return;
    }
