...
export function toClientState(s: GameState, viewerId?: string): ClientState {
  ...
  let arena = rest.arena ? { ...rest.arena, secret: {} } : null;
  // "Esquivá el rayo": mientras se elige, cada uno ve sólo su casilla (de los demás, sólo si ya eligieron)
  if (arena && arena.game === 'rayo' && arena.data.phase === 'pick') {
    const picks = (arena.data.picks ?? {}) as Record<string, number>;
    const own: Record<string, number> = {};
    if (viewerId && picks[viewerId] !== undefined) own[viewerId] = picks[viewerId];
    arena = { ...arena, data: { ...arena.data, picks: own, picked: Object.keys(picks) } };
  }
