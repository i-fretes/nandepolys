    ...
    try {
      switch (GAME) {
        case 'cartas':
          ...
          break;
        case 'trivia': if (d.reveal === null && d.answered?.[id] === undefined && Math.random() < 0.5) await p.locator(`[data-answer="${Math.floor(Math.random() * 4)}"]`).click({ force: true, timeout: 500 }); break;
...
if (GAME === 'trivia') {
  // El podio de la trivia muestra cómo le fue a cada uno y qué puso en cada pregunta
  const resumen = await a.locator('[data-trivia-resumen]').count();
  console.log(resumen ? '✔ el podio muestra el resumen de la trivia' : '✘ falta el resumen de la trivia');
  if (resumen) {
    await a.locator('[data-trivia-resumen] button').first().click({ force: true });
    await a.waitForTimeout(400);
    await a.screenshot({ path: `${OUT}/${GAME}-resumen.png` });
    const preguntas = await a.locator('[data-trivia-resumen] > div > div').count();
    console.log(`  preguntas en el resumen: ${preguntas}`);
  }
}
