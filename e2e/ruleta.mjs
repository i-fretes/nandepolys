...
for (let i = 0; i < ROUNDS; i++) {
  ...
  const under = await p.evaluate(() => {
    ...
    return {
      ...
      sameCell: at === win,
      // Lo que se ve en el centro de la ventana (no sólo bajo la línea del marcador): antes el
      // carrete frenaba corrido y se veía una celda verde al lado aunque el resultado fuera rojo
      centrado: (() => {
        const w = document.querySelector('.reel-window').getBoundingClientRect();
        const cx = w.left + w.width / 2;
        const r = win.getBoundingClientRect();
        return { desvio: Math.round(Math.abs((r.left + r.width / 2) - cx)), ancho: Math.round(r.width) };
      })(),
    };
  });
  ...
  ok(under && under.centrado.desvio <= 4, `tirada ${i + 1}: la celda que salió queda centrada bajo el marcador (desvío ${under && under.centrado.desvio} px)`);
